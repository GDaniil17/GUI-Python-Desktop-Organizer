...
from os import getcwd, makedirs, rename, mkdir, listdir
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        self.listOfExceptions = {'Cleaner.exe', 'Cleaner.pyw', 'Extensions.txt', 'extensions.txt', 'Cleaner.py'}
        ...
        if 1:
        	try:
		        ...
		        for i in s:
		        	...
		        	tmp_l = len(tmp_filename)
		        	i = ""
		        	for j, tmp in enumerate(tmp_filename, 1):
		        		if j != tmp_l:
		        			i += tmp
		        		else:
		        			i += "." + tmp_filename[-1].lower()
		        	t = u""+i.split()

		        	self.listOfExceptions.add(t[len(t)-1])
		        	print(self.listOfExceptions)
	        except:pass

        #self.listOfExceptions.update(

    def setExtensions(self):
        if exists(f"/Users/{self.cur}/Desktop/Extensions.txt"):
        ...
        elif exists(f"/Users/{self.cur}/Desktop/{self.cur}/Extensions.txt"):
        ...
        elif exists(f"/Users/{self.cur}/Desktop/extensions.txt"):
            f = open(f"/Users/{self.cur}/Desktop/extensions.txt", "r")
            s = f.read().replace(" ", "").replace('"', "").replace(",", "").replace("\n", ",")
            while len(s) != 0:
                    s = dict(map(lambda x: x.split('-'), s.split(',')))
    ...
    def on_modified(self):
            cur_folder_name = r"\Users\ " + self.cur + r"\Desktop"
            cur_folder_name = cur_folder_name.replace(" ", "")
            if cur_folder_name not in getcwd():
                return
            ...
            self.old = set()
            for filename in listdir(self.folder_to_track):
                i = 1
                ...
                if filename not in self.listOfExceptions:
                    try:                      
                        ...
                        extension = 'noname'
                        try:
                            extension = str(splitext(self.folder_to_track + '/' + filename)[1])
                        ...
                        year_exists = False
                        #month_exists = False
                        for folder_name in listdir(self.extensions_folders[extension]):
                            if folder_name == year:
                                ...
                                self.folder_destination_path = self.extensions_folders[extension] + "/" + year# + "/" + month
                                        #month_exists = True
                        if not year_exists:
                            mkdir(self.extensions_folders[extension] + "/" + year)
                            self.folder_destination_path = self.extensions_folders[extension] + "/" + year
                        file_exists = isfile(self.folder_destination_path + "/" + new_name)
                        while file_exists:
                            i += 1
                            new_name = splitext(self.folder_to_track + '/' + filename)[0] + str(i) + splitext(self.folder_to_track + '/' + filename)[1]
